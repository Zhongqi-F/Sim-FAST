...
stiffFactor=1000;
barA=5*10^(-3)*5*10^(-3);
barE=200*10^6; % Young's modulus

% perturbation Level 
perturbationLevel=0.5*10^(-3);
...
%% Design start
for step=1:5
    % we will try to apply a simple genetic algorithm to achieve the
    % inverse design process. We appy random perturbation to the node and
    % see if the node gives us better responses. If no, we reject the
    % perturbation. If the total energy curve is improved, we accepte the
    % perturbation. 
    ...
    %% Solve the folding angle and energy
    
    energyHis=zeros(sf.increStep,3);
    % 1 is bar energy 2 is rot spr energy 3 is total
    stressFreeAngleHis=zeros(sf.increStep,1);
    
    for i=1:sf.increStep   
        % set the stress free angle to solve for the total energy
        ...
        spr.SolveFK(node,squeeze(Uhis(i,:,:)));
    
        energyHis(i,1)=sum(bar.currentStrainEnergy_Vec);
        energyHis(i,2)=sum(spr.currentStrainEnergy_Vec);
        energyHis(i,3)=energyHis(i,1)+energyHis(i,2);
    end
    
    figure 
    hold on
    plot(stressFreeAngleHis,energyHis(:,1));
    plot(stressFreeAngleHis,energyHis(:,2));
    plot(stressFreeAngleHis,energyHis(:,3));
    legend('bar','spr','total');
    hold off

    if energyHis(150,3) < energyHis(200,3)
        node.coordinates_Mat(12,:)=node.coordinates_Mat(12,:)-v_perturb1';
        node.coordinates_Mat(13,:)=node.coordinates_Mat(13,:)-v_perturb2';
    end

    % We want to reset the loading information here before starting the new
    % step of genetic algorithm
    spr.theta_StressFree_Vec(1)=originalRot(1);
    bar.SolveFK(node,zeros(24,3));
    spr.SolveFK(node,zeros(24,3));
...
%% Solve the folding angle and energy for final results    
