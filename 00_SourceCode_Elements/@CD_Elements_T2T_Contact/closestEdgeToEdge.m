...
[A, oTriAPoint, oTriBPoint] = obj.simdSegmentSegment2(iTriAEdges(1,:), iTriBEdge);

%Test to see if the distances found so far were the closest
separatingDir = oTriBPoint - oTriAPoint;
oIsFinished = oIsFinished ||...
    obj.closestEdgePoints(iTriAEdges(2, 1:3), oTriAPoint,...
...
[B, A2p, B2p] = obj.simdSegmentSegment2(iTriAEdges(2,:), iTriBEdge);
separatingDir = B2p - A2p;
oIsFinished = oIsFinished ||...
    obj.closestEdgePoints(iTriAEdges(3,1:3), A2p,...
...
[C, A3p, B3p] = obj.simdSegmentSegment2(iTriAEdges(3,:), iTriBEdge);
separatingDir = B3p - A3p;
oIsFinished = oIsFinished ||...
    obj.closestEdgePoints(iTriAEdges(1, 1:3), A3p,...
