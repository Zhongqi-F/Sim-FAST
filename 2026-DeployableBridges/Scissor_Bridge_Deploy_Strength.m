...
DepStep = max(1, int32((1 - DepRate) * 2000));
...
% Use the same REFERENCE coordinates as the deploy script (fully deployed state)
...
% Boundary conditions:
%   Left end  (nodes 1,2): fully pinned - restrain x, y, z
%   Right end (nodes 8*N+1, 8*N+2): roller - restrain z only
%   Top nodes (3,4) are released - fixing them causes singularity
lastNode1 = 8*N+1;   % right end bottom front
...
for i = 1:50
    ...
    %  This occurs when DepRate is too small (near-folded geometry)
    % -------------------------------------------------------------------
...
% -----------------------------------------------------------------------
if isnan(maxDCR_val)
    % Solver failed (singular matrix) — report and skip plots
    fprintf('\n========================================\n');
    ...
    fprintf('  Try DepRate >= 0.3 for this bridge type.\n');
    fprintf('========================================\n');
    fprintf('-----------------------------\n');
    ...
    % Solver succeeded — compute and report all results
