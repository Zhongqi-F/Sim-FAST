function [Kspr]=Solve_Global_Stiff(obj,node,U,Mspr,Cspr)
    ...
    part2ij_temp_1=-m1.*rkj./(m_square.*rkj_norm)...
        +rkj_norm./(m_square.*m_square).*((m1.*cross_rkj_rij_m)...
        +(cross_rkj_rij_m(:,1).*m));
    part2ij_temp_2=-m2.*rkj./(m_square.*rkj_norm)...
        +rkj_norm./(m_square.*m_square).*((m2.*cross_rkj_rij_m)...
        +(cross_rkj_rij_m(:,2).*m));
    part2ij_temp_3=-m3.*rkj./(m_square.*rkj_norm)...
        +rkj_norm./(m_square.*m_square).*((m3.*cross_rkj_rij_m)...
        +(cross_rkj_rij_m(:,3).*m));
    ...
    part2lk_temp_1=-(n1.*rkj./(n_square.*rkj_norm))...
        -rkj_norm./(n_square.*n_square).*((n1.*cross_rkj_rkl_n)...
        +(cross_rkj_rkl_n(:,1).*n));
    part2lk_temp_2=-(n2.*rkj./(n_square.*rkj_norm))...
        -rkj_norm./(n_square.*n_square).*((n2.*cross_rkj_rkl_n)...
        +(cross_rkj_rkl_n(:,2).*n));
    part2lk_temp_3=-(n3.*rkj./(n_square.*rkj_norm))...
        -rkj_norm./(n_square.*n_square).*((n3.*cross_rkj_rkl_n)...
        +(cross_rkj_rkl_n(:,3).*n));
