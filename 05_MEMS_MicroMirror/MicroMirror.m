...
bar.barConnect_Mat=[bar.barConnect_Mat; 2*(BeamSegment+1)+2,2*(BeamSegment+1)+3];

bar.barConnect_Mat=[bar.barConnect_Mat; 1,2+(BeamSegment+1)];
bar.barConnect_Mat=[bar.barConnect_Mat; 2*(BeamSegment+1)+2,3*(BeamSegment+1)+2+1];

for i=1:BeamSegment

    bar.barConnect_Mat=[bar.barConnect_Mat; i+1,i+2];    
    bar.barConnect_Mat=[bar.barConnect_Mat; BeamSegment+1+1+i,BeamSegment+1+1+i+1];    
    bar.barConnect_Mat=[bar.barConnect_Mat; 2*(BeamSegment+1)+2+i,2*(BeamSegment+1)+3+i];    
    bar.barConnect_Mat=[bar.barConnect_Mat; 3*(BeamSegment+1)+2+i,3*(BeamSegment+1)+3+i];
...
for i=1:BeamSegment+1
    bar.barConnect_Mat=[bar.barConnect_Mat; 1+i,(BeamSegment+1)+1+i];    
    bar.barConnect_Mat=[bar.barConnect_Mat; 2*(BeamSegment+1)+2+i,3*(BeamSegment+1)+2+i];
end
...
rotSpr.rotSprIJKL_Mat=[rotSpr.rotSprIJKL_Mat; 1,2,...
    (BeamSegment+1)+1+1,(BeamSegment+1)+2+1];
rotSpr.rotSprIJKL_Mat=[rotSpr.rotSprIJKL_Mat;2*(BeamSegment+1)+1+1,...
    2*(BeamSegment+1)+2+1,3*(BeamSegment+1)+2+1,3*(BeamSegment+1)+3+1];

for i=2:BeamSegment

    if mod(i,2)==1
        rotSpr.rotSprIJKL_Mat=[rotSpr.rotSprIJKL_Mat; (BeamSegment+1)+i,1+i,...
            (BeamSegment+1)+1+i,(BeamSegment+1)+2+i];
        rotSpr.rotSprIJKL_Mat=[rotSpr.rotSprIJKL_Mat;3*(BeamSegment+1)+1+i,...
            2*(BeamSegment+1)+2+i,3*(BeamSegment+1)+2+i,3*(BeamSegment+1)+3+i];
    else
        rotSpr.rotSprIJKL_Mat=[rotSpr.rotSprIJKL_Mat; i,1+i,...
            (BeamSegment+1)+1+i,2+i];
        rotSpr.rotSprIJKL_Mat=[rotSpr.rotSprIJKL_Mat;2*(BeamSegment+1)+1+i,...
...
rotSpr.rotSprK_Vec=[rotSpr.rotSprK_Vec;SprBeam*ones((BeamSegment+1)*2,1)];

baseSprNum=[1,2,2*BeamSegment+1,2*BeamSegment+2];
rotSpr.rotSprK_Vec(baseSprNum)=rotSpr.rotSprK_Vec(baseSprNum)*2;

rotSpr.theta_StressFree_Vec=[rotSpr.theta_StressFree_Vec;pi*ones((BeamSegment+1)*2,1)];
...
for i=1:BeamSegment
    if mod(i,2)==1
        bar.barConnect_Mat=[bar.barConnect_Mat; i+1,(BeamSegment+1)+2+i];
        bar.barConnect_Mat=[bar.barConnect_Mat; 2*(BeamSegment+1)+2+i,...
            3*(BeamSegment+1)+3+i];
    else
        bar.barConnect_Mat=[bar.barConnect_Mat; i+2,(BeamSegment+1)+1+i];
        bar.barConnect_Mat=[bar.barConnect_Mat; 2*(BeamSegment+1)+3+i,...
            3*(BeamSegment+1)+2+i];
    end
...
for i=1:BeamSegment
    if mod(i,2)==1
        ...
        rotSpr.rotSprIJKL_Mat=[rotSpr.rotSprIJKL_Mat;i+1,...
...
rotSpr.rotSprK_Vec=[rotSpr.rotSprK_Vec;SprBeamDiag*ones(BeamSegment*2,1)];
rotSpr.theta_StressFree_Vec=[rotSpr.theta_StressFree_Vec;pi*ones(BeamSegment*2,1)];


%% Define bar for the panels (perimeter)
bar.barConnect_Mat=[bar.barConnect_Mat; 2*(BeamSegment+1)+1,4*(BeamSegment+1)+2];
bar.barConnect_Mat=[bar.barConnect_Mat; 4*(BeamSegment+1)+2+5,4*(BeamSegment+1)+2+6];

bar.E_Vec=[bar.E_Vec;Eeq*ones(2,1)];
bar.A_Vec=[bar.A_Vec;Apanel*ones(2,1)];
bar.L0_Vec=[bar.L0_Vec;L3*ones(2,1)];

bar.barConnect_Mat=[bar.barConnect_Mat; (BeamSegment+1)+1,4*(BeamSegment+1)+2+1];
bar.barConnect_Mat=[bar.barConnect_Mat; 3*(BeamSegment+1)+2,4*(BeamSegment+1)+2+2];

bar.E_Vec=[bar.E_Vec;Eeq*ones(2,1)];
bar.A_Vec=[bar.A_Vec;Apanel*ones(2,1)];
bar.L0_Vec=[bar.L0_Vec;L1*ones(2,1)];

bar.barConnect_Mat=[bar.barConnect_Mat; 4*(BeamSegment+1)+2+1,4*(BeamSegment+1)+2+4];
bar.barConnect_Mat=[bar.barConnect_Mat; 4*(BeamSegment+1)+2+2,4*(BeamSegment+1)+2+7];

bar.E_Vec=[bar.E_Vec;Eeq*ones(2,1)];
bar.A_Vec=[bar.A_Vec;Apanel*ones(2,1)];
bar.L0_Vec=[bar.L0_Vec;L4*ones(2,1)];

bar.barConnect_Mat=[bar.barConnect_Mat; 4*(BeamSegment+1)+2+4,4*(BeamSegment+1)+2+5];
bar.barConnect_Mat=[bar.barConnect_Mat; 4*(BeamSegment+1)+2+6,4*(BeamSegment+1)+2+7];
...
bar.barConnect_Mat=[bar.barConnect_Mat; BeamSegment+2,4*(BeamSegment+1)+2+4];
bar.barConnect_Mat=[bar.barConnect_Mat; 3*(BeamSegment+1)+2,4*(BeamSegment+1)+2+7];

bar.E_Vec=[bar.E_Vec;Eeq*ones(2,1)];
bar.A_Vec=[bar.A_Vec;Apanel*ones(2,1)];
bar.L0_Vec=[bar.L0_Vec;sqrt(L1*L1+L4*L4)*ones(2,1)];

bar.barConnect_Mat=[bar.barConnect_Mat; BeamSegment+2,4*(BeamSegment+1)+2+5];
bar.barConnect_Mat=[bar.barConnect_Mat; 3*(BeamSegment+1)+2,4*(BeamSegment+1)+2+6];

bar.E_Vec=[bar.E_Vec;Eeq*ones(2,1)];
bar.A_Vec=[bar.A_Vec;Apanel*ones(2,1)];
bar.L0_Vec=[bar.L0_Vec;sqrt(L2*L2+L4*L4)*ones(2,1)];

bar.barConnect_Mat=[bar.barConnect_Mat; 2*(BeamSegment+1)+1,4*(BeamSegment+1)+2+5];
bar.barConnect_Mat=[bar.barConnect_Mat; 4*(BeamSegment+1)+2,4*(BeamSegment+1)+2+6];

bar.E_Vec=[bar.E_Vec;Eeq*ones(2,1)];
bar.A_Vec=[bar.A_Vec;Apanel*ones(2,1)];
bar.L0_Vec=[bar.L0_Vec;L4*ones(2,1)];

bar.barConnect_Mat=[bar.barConnect_Mat; 2*(BeamSegment+1)+1,4*(BeamSegment+1)+2+3];
bar.barConnect_Mat=[bar.barConnect_Mat; 4*(BeamSegment+1)+2,4*(BeamSegment+1)+2+3];
bar.barConnect_Mat=[bar.barConnect_Mat; 4*(BeamSegment+1)+2+3,4*(BeamSegment+1)+2+5];
bar.barConnect_Mat=[bar.barConnect_Mat; 4*(BeamSegment+1)+2+3,4*(BeamSegment+1)+2+6];
...
node.currentExtForce_Mat=zeros(4*(BeamSegment+1)+2+7,3);
node.currentExtForce_Mat(:,3)=-node.mass_Vec*9.8/sqrt(2);
node.currentExtForce_Mat(:,1)=node.mass_Vec*9.8/sqrt(2);
...
% Sine wave input
time=(1:step)*caa.dt;
sprNum=size(rotSpr.rotSprK_Vec);
sprNum=sprNum(1);
...
for i=1:step
    caa.Fext(i,:,:)=node.currentExtForce_Mat;
end
