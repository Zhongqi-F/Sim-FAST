...
for i=1:lockCreaseNum
    ...
    for j=unusedLocks'
        ...
        previousLock=zlspr.node_ij_mat((74+2*i-1):(74+2*i),:);
            
        zlspr.node_ij_mat(74+2*i-1,:)=fold_lock_number(j,1:2);
        zlspr.node_ij_mat(74+2*i,:)=fold_lock_number(j,3:4);  
        ...
        if currentStiff<previousStiff
            zlspr.node_ij_mat((74+2*i-1):(74+2*i),:)=previousLock;
        else
...
randomLock=randperm(25,lockCreaseNum);

for i=1:lockCreaseNum
    zlspr.node_ij_mat(74+2*i-1,:)=fold_lock_number(randomLock(i),1:2);
    zlspr.node_ij_mat(74+2*i,:)=fold_lock_number(randomLock(i),3:4);  
end
...
UhisStoreLock5=UhisStoreLock{3};
dispHisLock=squeeze(UhisStoreLock5(:,loadIndex,3));
dispHisLock=mean(dispHisLock,2);

UhisStoreLock1=UhisStoreLock{1};
dispHisLock1=squeeze(UhisStoreLock1(:,loadIndex,3));
dispHisLock1=mean(dispHisLock1,2);

dispHisLock=-[0;dispHisLock];
dispHisLock1=-[0;dispHisLock1];
...
plot(dispHisLock1,loadHis)
plot(dispHisRandomLock,loadHis)
plot(dispHisNonLock,loadHis)
legend('With 3 Locks (Optimized)','With 1 Locks (Optimized)'...
    ,'With 3 Locks (Random)','Without Locks')
